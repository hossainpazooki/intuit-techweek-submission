...
def _find_harness_src():
    """Locate the closed-loop-default-detection harness `src/` robustly.

    The harness is a sibling project; depending on how the repos are laid out on
    a given machine it may live next to this repo, one level up, or two. We probe
    a few candidate roots and fall back to the immediate-sibling path (the C term
    degrades to best-effort ~0 if none resolve, never crashes).
    """
    name = "closed-loop-default-detection"
    candidates = [
        ROOT.parent / name / "src",          # sibling of this repo
        ROOT.parent.parent / name / "src",   # repo nested one level (e.g. under a wrapper dir)
        ROOT.parent.parent.parent / name / "src",
    ]
    for c in candidates:
        if (c / "cldd" / "__init__.py").exists():
            return c
    return candidates[0]


HARNESS_SRC = _find_harness_src()
